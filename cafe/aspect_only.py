...
class DualHeadBert(nn.Module):
    def __init__(self, model_name, aspect_only=True, global_pos_weights=None,sentiment_class_weights=None):
        ...
        self.sentiment_loss_weight=3.0
    ...
    def forward(self, input_ids, attention_mask, labels=None, class_weights=None):
        ...
        # 🆕 수정: sentiment_intermediate를 거쳐야 함
        sentiment_features = self.sentiment_intermediate(output)  # ← 이 줄 추가
        sentiment_logits = self.sentiment_head(sentiment_features).view(-1, 12, 3)  # ← output을 sentiment_features로 변경
        ...
        if labels is not None:
            ...
            if self.aspect_only:
                ...
                sentiment_loss_fct = FocalLoss(gamma=1.0, weight=weights_to_use)  # gamma 2.0 → 2.5
                sentiment_loss = 0
...
if __name__ == "__main__":
    ...
    if RUN_STAGE_3:
        # ============================================================
        # STAGE 3: 감정 분류 학습 (단일 측면)
        # ============================================================
        print("\n" + "=" * 80)
        print("🎯 STAGE 3: 감정 분류 학습 - 단일 측면")
        print("=" * 80 + "\n")
        ...
        if torch.cuda.is_available():
            ...
            print("⚠️ CPU 사용 중")
        # 3-1. 단일 측면 데이터 로드
        print("\n📂 [STAGE 3] 단일 측면 데이터 로딩 중...")
        try:
            df_train_single = pd.read_csv('split_train_single.csv', dtype={'label': str})
            df_val_single = pd.read_csv('split_val_single.csv', dtype={'label': str})
            df_test_single = pd.read_csv('split_test_single.csv', dtype={'label': str})

            train_texts_single = df_train_single['Original_Review'].tolist()
            train_labels_single = df_train_single['label'].tolist()
            val_texts_single = df_val_single['Original_Review'].tolist()
            val_labels_single = df_val_single['label'].tolist()
            test_texts_single = df_test_single['Original_Review'].tolist()
            test_labels_single = df_test_single['label'].tolist()
            ...
            print(f"❌ 파일을 찾을 수 없습니다: {e}")
            print("💡 먼저 STAGE 1-2를 실행하여 데이터를 생성해주세요.")
            exit()

        # 3-2. 감정별 Class Weight 계산
        print("\n📊 [STAGE 3] 감정별 Class Weight 계산 중...")
        ...
        print(f"\n단일 측면 감정 분포:")
        print(f"  긍정: {sentiment_counts[1]:5d}개 ({sentiment_counts[1] / total_sentiments * 100:5.1f}%)")
        print(f"  부정: {sentiment_counts[2]:5d}개 ({sentiment_counts[2] / total_sentiments * 100:5.1f}%)")
        print(f"  중립: {sentiment_counts[3]:5d}개 ({sentiment_counts[3] / total_sentiments * 100:5.1f}%)")

        sentiment_weights = np.zeros(3)
        for i in range(3):
            sentiment_idx = i + 1
            ratio = sentiment_counts[sentiment_idx] / total_sentiments
            if ratio > 0:
                base_weight = (1 - ratio) / ratio
                sentiment_weights[i] = max(1.0, min(base_weight, 20.0))
            else:
                sentiment_weights[i] = 10.0

        print(f"\n📈 계산된 감정 Class Weight:")
        print(f"  긍정: {sentiment_weights[0]:6.2f}")
        print(f"  부정: {sentiment_weights[1]:6.2f}")
        print(f"  중립: {sentiment_weights[2]:6.2f}")

        sentiment_class_weights_single = torch.tensor(sentiment_weights, dtype=torch.float32).to(device)

        if 'tokenizer'  not in locals():
            print("\n🔧 [STAGE 3] Tokenizer 초기화 중...")
            tokenizer = BertTokenizer.from_pretrained(MODEL_NAME)
            print("✅ Tokenizer 초기화 완료!")
        # 3-3. 데이터셋 준비
        train_dataset_single = CafeAspectDataset(train_texts_single, train_labels_single, tokenizer, MAX_LEN)
        val_dataset_single = CafeAspectDataset(val_texts_single, val_labels_single, tokenizer, MAX_LEN)

        # 3-4. STAGE 2 모델 로드
        print(f"\n🔧 [STAGE 3] STAGE 2 모델 로딩 중... ()")

        if not os.path.exists(f"{SAVE_PATH}/model_state_dict.pt"):
            print(f"❌ STAGE 2 모델을 찾을 수 없습니다: {SAVE_PATH}/model_state_dict.pt")
            print("💡 먼저 STAGE 2를 실행해주세요.")
            exit()

        # pos_weight 로드
        global_pos_weights = torch.load(f"{SAVE_PATH}/global_pos_weights.pt", weights_only=True)

        # STAGE 2 모델 로드 (aspect_only=True)
        model_stage2 = DualHeadBert(
            MODEL_NAME,
            aspect_only=True,
            global_pos_weights=global_pos_weights
        ).to(device)

        model_stage2.load_state_dict(torch.load(f"{SAVE_PATH}/model_state_dict.pt", weights_only=False))
        print("✅ STAGE 2 모델 로드 완료!")

        # 3-5. STAGE 3 모델 생성 (aspect_only=False)
        print("\n🔧 [STAGE 3] 감정 분류 모델 초기화 중...")

        model_sentiment_single = DualHeadBert(
            MODEL_NAME,
            aspect_only=False,
            global_pos_weights=global_pos_weights,
            sentiment_class_weights=sentiment_class_weights_single
        ).to(device)

        # STAGE 2 가중치 복사
        model_sentiment_single.load_state_dict(model_stage2.state_dict(), strict=False)
        print("✅ STAGE 2 가중치 로드 완료!")

        # sentiment_head 활성화
        for param in model_sentiment_single.sentiment_head.parameters():
            param.requires_grad = True
        print("✅ sentiment_head 활성화 완료!")

        # aspect_head 동결
        for param in model_sentiment_single.aspect_head.parameters():
            param.requires_grad = False
        print("🔒 aspect_head 동결 완료!")

        # BERT 상위 레이어만 활성화
        for name, param in model_sentiment_single.bert.named_parameters():
            # if 'encoder.layer.11' in name or 'encoder.layer.10' in name or 'encoder.layer.9' in name:
            #     param.requires_grad = True
            if any(f'encoder.layer.{i}' in name for i in [11, 10, 9, 8, 7, 6]):
                param.requires_grad = True
            else:
                param.requires_grad = False
        print("🔓 BERT 상위 6개 레이어만 활성화!")

        # 3-6. Sampler 생성
        print("\n⚖️ [STAGE 3] Sentiment Sampler 생성 중...")
        ...
        for label_str in train_labels_single:
            ...
            if has_negative:
                current_weight += sentiment_weights[1] / 5.0  # 부정 강화
            if has_neutral:
                current_weight += sentiment_weights[2] / 5.0  # 중립 강화

            sample_weights_single.append(current_weight)

        sampler_sentiment_single = WeightedRandomSampler(
            weights=sample_weights_single,
            num_samples=len(sample_weights_single),
            replacement=True
        )
        print("✅ Sentiment Sampler 생성 완료")

        # 3-7. 학습 설정
        training_args_single = TrainingArguments(
            output_dir="./checkpoints_sentiment_single",
            num_train_epochs=5,
            per_device_train_batch_size=BATCH_SIZE,
            per_device_eval_batch_size=BATCH_SIZE,
            learning_rate=5e-5,
            warmup_steps=100,
            weight_decay=0.01,
            logging_dir='./logs_sentiment_single',
            logging_steps=50,
            eval_strategy="epoch",
            save_strategy="epoch",
            load_best_model_at_end=True,
        ...
        # 3-8. 학습 시작
        print(f"\n🚀 [STAGE 3] 학습 시작 (Epoch: 5)...")

        trainer_sentiment_single = CustomTrainer(
            model=model_sentiment_single,
            args=training_args_single,
            train_dataset=train_dataset_single,
            eval_dataset=val_dataset_single,
            train_sampler=sampler_sentiment_single,
            class_weights=sentiment_class_weights_single,
            aspect_only=False
        )

        trainer_sentiment_single.train()

        # 3-9. 모델 저장
        print(f"\n💾 [STAGE 3] 모델 저장 중... ({SAVE_PATH_SENTIMENT_SINGLE})")
        os.makedirs(SAVE_PATH_SENTIMENT_SINGLE, exist_ok=True)
        torch.save(model_sentiment_single.state_dict(), f"{SAVE_PATH_SENTIMENT_SINGLE}/model_state_dict.pt")
        ...
        torch.save(sentiment_class_weights_single, f"{SAVE_PATH_SENTIMENT_SINGLE}/sentiment_class_weights.pt")
        print("✅ STAGE 3 모델 저장 완료!")

    if RUN_STAGE_4:
        # ============================================================
        # STAGE 4: 감정 분류 학습 (단일 + 복합 측면)
        # ============================================================
        print("\n" + "=" * 80)
        print("🎯 STAGE 4: 감정 분류 학습 - 단일 + 복합 측면")
        print("=" * 80 + "\n")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        if torch.cuda.is_available():
            print(f"🎮 GPU 사용: {torch.cuda.get_device_name(0)}")
        ...
        print("\n📂 [STAGE 4] 결합 데이터 로딩 중...")
        try:
            df_train_combined = pd.read_csv('split_train_combined.csv', dtype={'label': str})
            df_val_combined = pd.read_csv('split_val_combined.csv', dtype={'label': str})
            df_test_combined = pd.read_csv('split_test_combined.csv', dtype={'label': str})

            train_texts_combined = df_train_combined['Original_Review'].tolist()
            train_labels_combined = df_train_combined['label'].tolist()
            val_texts_combined = df_val_combined['Original_Review'].tolist()
            val_labels_combined = df_val_combined['label'].tolist()
            test_texts_combined = df_test_combined['Original_Review'].tolist()
            test_labels_combined = df_test_combined['label'].tolist()

            print(f"✅ 학습 데이터: {len(train_texts_combined)}개")
            print(f"✅ 검증 데이터: {len(val_texts_combined)}개")
            print(f"✅ 테스트 데이터: {len(test_texts_combined)}개")
        except FileNotFoundError as e:
            print(f"❌ 파일을 찾을 수 없습니다: {e}")
            print("💡 먼저 STAGE 2를 실행하여 데이터를 생성해주세요.")
            exit()

        # 4-2. 감정별 Class Weight 재계산
        print("\n📊 [STAGE 4] 감정별 Class Weight 재계산 중...")

        sentiment_counts_combined = {1: 0, 2: 0, 3: 0}
        ...
        for label_str in train_labels_combined:
            ...
            for i in range(12):
                sentiment = int(label[i])
                if sentiment > 0:
                    sentiment_counts_combined[sentiment] = sentiment_counts_combined.get(sentiment, 0) + 1

        total_sentiments_combined = sum(sentiment_counts_combined.values())

        print(f"\n결합 데이터 감정 분포:")
        print(
            f"  긍정: {sentiment_counts_combined[1]:5d}개 ({sentiment_counts_combined[1] / total_sentiments_combined * 100:5.1f}%)")
        print(
            f"  부정: {sentiment_counts_combined[2]:5d}개 ({sentiment_counts_combined[2] / total_sentiments_combined * 100:5.1f}%)")
        print(
            f"  중립: {sentiment_counts_combined[3]:5d}개 ({sentiment_counts_combined[3] / total_sentiments_combined * 100:5.1f}%)")

        sentiment_weights_combined = np.zeros(3)
        for i in range(3):
            sentiment_idx = i + 1
            ratio = sentiment_counts_combined[sentiment_idx] / total_sentiments_combined

            if ratio > 0:
                base_weight = (1 - ratio) / ratio
                # 이 부분 전체 삭제 (추가 배수 제거)
                # if sentiment_idx == 2:  # 부정
                #     base_weight *= 1.5
                # elif sentiment_idx == 3:  # 중립
                #     base_weight *= 2.0

                sentiment_weights_combined[i] = max(1.0, min(base_weight, 20.0))
            else:
                sentiment_weights_combined[i] = 10.0

        print(f"\n📈 계산된 감정 Class Weight:")
        print(f"  긍정: {sentiment_weights_combined[0]:6.2f}")
        print(f"  부정: {sentiment_weights_combined[1]:6.2f}")
        print(f"  중립: {sentiment_weights_combined[2]:6.2f}")

        sentiment_class_weights_combined = torch.tensor(sentiment_weights_combined, dtype=torch.float32).to(device)
        ...
        # 4-4. STAGE 3 모델 로드
        print(f"\n🔧 [STAGE 4] STAGE 3 모델 로딩 중... ({SAVE_PATH_SENTIMENT_SINGLE})")

        if not RUN_STAGE_3 and not os.path.exists(f"{SAVE_PATH_SENTIMENT_SINGLE}/model_state_dict.pt"):
            print(f"❌ STAGE 3 모델을 찾을 수 없습니다: {SAVE_PATH_SENTIMENT_SINGLE}/model_state_dict.pt")
            print("💡 먼저 STAGE 3을 실행해주세요.")
            exit()

        # pos_weight 로드
        if RUN_STAGE_3:
            # STAGE 3 방금 완료됨
            model_sentiment_combined = model_sentiment_single
        else:
            # STAGE 3 모델 로드
            global_pos_weights = torch.load(f"{SAVE_PATH_SENTIMENT_SINGLE}/global_pos_weights.pt",
                                            weights_only=True)
            sentiment_class_weights_single = torch.load(f"{SAVE_PATH_SENTIMENT_SINGLE}/sentiment_class_weights.pt",
                                                        weights_only=True)

            model_sentiment_combined = DualHeadBert(
                MODEL_NAME,
                aspect_only=False,
                global_pos_weights=global_pos_weights,
                sentiment_class_weights=sentiment_class_weights_single
            ...
            model_sentiment_combined.load_state_dict(
                torch.load(f"{SAVE_PATH_SENTIMENT_SINGLE}/model_state_dict.pt", weights_only=False),
                strict=False
            )
            print("✅ STAGE 3 모델 로드 완료!")

        # sentiment_class_weights 업데이트
        model_sentiment_combined.sentiment_class_weights = sentiment_class_weights_combined
        print("✅ Class Weights 업데이트 완료!")

        # 4-5. Sampler 생성
        print("\n⚖️ [STAGE 4] Sentiment Sampler 생성 중...")
        ...
        for label_str in train_labels_combined:
            ...
            for i in range(12):
                if label_list[i] > 0:
                    current_weight += 1.0  # 감정 종류 무관하게 동일

            # has_negative = any(label_list[i] == 2 for i in range(12))
            # has_neutral = any(label_list[i] == 3 for i in range(12))
            #
            # if has_negative:
            #     current_weight += sentiment_weights_combined[1] / 2.0
            # if has_neutral:
            #     current_weight += sentiment_weights_combined[2] / 2.0

            sample_weights_combined.append(current_weight)

        sampler_sentiment_combined = WeightedRandomSampler(
            weights=sample_weights_combined,
            num_samples=len(sample_weights_combined),
            replacement=True
        )
        ...
        # 4-6. 학습 설정
        training_args_combined = TrainingArguments(
            output_dir="./checkpoints_sentiment_combined",
            num_train_epochs=10,
            per_device_train_batch_size=BATCH_SIZE,
            per_device_eval_batch_size=BATCH_SIZE,
            learning_rate=2e-5,
            warmup_steps=50,
            weight_decay=0.01,
            logging_dir='./logs_sentiment_combined',
            logging_steps=50,
            eval_strategy="epoch",
            save_strategy="epoch",
            load_best_model_at_end=True,
        ...
        # 4-7. 학습 시작
        print(f"\n🚀 [STAGE 4] 학습 시작 (Epoch: 3)...")

        trainer_sentiment_combined = CustomTrainer(
            model=model_sentiment_combined,
            args=training_args_combined,
            train_dataset=train_dataset_combined,
            eval_dataset=val_dataset_combined,
            train_sampler=sampler_sentiment_combined,
            class_weights=sentiment_class_weights_combined,
            aspect_only=False
        )

        trainer_sentiment_combined.train()

        # 4-8. 최종 모델 저장
        print(f"\n💾 [STAGE 4] 최종 모델 저장 중... ({SAVE_PATH_SENTIMENT_FINAL})")
        os.makedirs(SAVE_PATH_SENTIMENT_FINAL, exist_ok=True)
        torch.save(model_sentiment_combined.state_dict(), f"{SAVE_PATH_SENTIMENT_FINAL}/model_state_dict.pt")
        ...
        torch.save(sentiment_class_weights_combined, f"{SAVE_PATH_SENTIMENT_FINAL}/sentiment_class_weights.pt")
        print("✅ 최종 모델 저장 완료!")
